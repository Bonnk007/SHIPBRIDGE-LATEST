...
export default function App() {
  ...
  async function handleFiles(files) {
    ...
    setLoading(true)
    try {
      // A CPI export may be a single iFlow ZIP or a whole package containing
      // multiple sub-processes. We try the iFlow parser first (which is the
      // common case); if it can't find an .iflw and the ZIP looks like a
      // package (has a scenarioflows/ tree or a metainfo.prop), fall through
      // to the package parser and load every sub-process as its own flow.
      const parsed = []
      for (const f of zips) {
        try {
          parsed.push(await parseZip(f))
        } catch (e) {
          if (!/No \.iflw/i.test(String(e.message || ''))) throw e
          const pkg = await parsePackageZip(f)
          const packageName = (f.name || 'package').replace(/\.zip$/i, '')
          // Each sub-process becomes a flow the engine understands, with the
          // package's bundled scripts/xslts/mappings/schemas attached so
          // dead-asset analysis works on all of them together.
          const bundled = {
            scripts: Object.keys(pkg.scripts || {}),
            xslts: Object.keys(pkg.xslts || {}),
            mappings: Object.keys(pkg.mappings || {}),
            schemas: Object.keys(pkg.schemas || {}),
          }
          const mappingSchemaRefs = {}
          for (const [name, parsedMmap] of Object.entries(pkg.mappings || {})) {
            if (parsedMmap.schemaRefs?.length) mappingSchemaRefs[name] = parsedMmap.schemaRefs
          }
          for (const [subId, sub] of Object.entries(pkg.subProcesses || {})) {
            parsed.push({
              id: `${packageName}-${subId}`.toLowerCase().replace(/[^a-z0-9-]/g, '-').slice(0, 60),
              name: sub.name || subId,
              iflwName: pkg.iflwPath ? pkg.iflwPath.split('/').pop().replace(/\.iflw$/i, '') : packageName,
              zipName: f.name || '',
              packageName,
              steps: sub.steps || {},
              edges: sub.edges || [],
              startId: sub.startId,
              entryPoints: sub.entryPoints || [],
              exitPoints: sub.exitPoints || [],
              adapterMap: sub.adapterMap || {},
              adapterList: sub.adapterList || [],
              bundled,
              mappingSchemaRefs,
              // v48.18.0 — carry parsed content so Intelligence can display it
              mappings: pkg.mappings || {},
              valueMappings: pkg.valueMappings || {},
              scriptContents: pkg.scripts || {},
              xsltContents: pkg.xslts || {},
            })
          }
        }
      }
  ...
  // Rename an iFlow in the registry — a temporary, in-session-only display
  // label so a user can tell which loaded flow is which without going back
  // to the ZIP filename. Purely cosmetic: the id doesn't change, so
  // dependency indexes, exports, and traces still point at the same flow.
  function renameFlow(id, newName) {
    setRegistry(prev => {
      const next = new Map(prev)
      const f = next.get(id)
      if (!f) return prev
      next.set(id, { ...f, name: newName || f.name })
      return next
    })
  }
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'auto' }}>
          ...
          {page === 'intel' && <Intelligence registry={registry} onUpload={handleFiles} onRename={renameFlow} onClear={clearAll} />}
