...
export const RESOURCE_KINDS = [
  ...
  'schema',        // XSD / EDMX / WSDL files referenced by adapters
...
export function flowResources(flow) {
  ...
  for (const step of Object.values(flow.steps || {})) {
    ...
    // Schema references from adapter config.
    // OData adapters carry `edmxFilePath` (e.g. "edmx/ORG.EDMX") or a bare
    // `resourcePath` (e.g. "YY1_IF_PAYMENT_DOCUMENTS"). We track the file
    // basename because that's what matches the bundled inventory keys.
    const rp = c.rawProps || {}
    const schemaRefs = [
      rp.edmxFilePath, rp.xsdName, rp.xsdPath, rp.wsdlName, rp.wsdlPath,
      rp.resourcePath, rp.schemaFilePath, rp.schemaPath,
    ]
    for (const s of schemaRefs) {
      const n = norm(s)
      if (!n) continue
      // "edmx/ORG.EDMX" → "ORG.EDMX"; keep bare names as-is.
      const base = n.includes('/') ? n.slice(n.lastIndexOf('/') + 1) : n
      add('schema', base)
    }
  }

  // Schema links inside message mappings — every .mmap references its source
  // and target XSD/EDMX via <lnk><key typeID="xsd"><elem>NAME.xsd</elem>...
  // These arrive on the parsed flow as `mappingSchemaRefs` per mapping name.
  for (const refs of Object.values(flow.mappingSchemaRefs || {})) {
    for (const ref of refs || []) add('schema', ref)
  }
...
export function deadAssets(idx) {
  ...
  // What's bundled across all flows — and, for each asset, WHICH flows bundle
  // it. Without the owner, a landscape-wide dead list just says "these files
  // are unused somewhere", which doesn't tell you where to go and delete them.
  const bundled = { scripts: new Map(), xslts: new Map(), mappings: new Map(), schemas: new Map() }
  const addBundled = (bucket, name, flow) => {
    const n = norm(name)
    if (!n) return
    if (!bucket.has(n)) bucket.set(n, new Set())
    bucket.get(n).add(flow.id)
  }
  for (const f of flows) {
    const b = f.bundled || {}
    ;(b.scripts  || []).forEach(n => addBundled(bundled.scripts,  n, f))
    ;(b.xslts    || []).forEach(n => addBundled(bundled.xslts,    n, f))
    ;(b.mappings || []).forEach(n => addBundled(bundled.mappings, n, f))
    ;(b.schemas  || []).forEach(n => addBundled(bundled.schemas,  n, f))
  }
  ...
  const refSchemas  = new Set(idx.index.get('schema').keys())

  // Each entry is { name, flows: [flowId] } — flows being the ones that ship
  // the unused file. Kept as objects rather than bare strings so callers can
  // show ownership; `.name` reads the same as the old string in a template.
  const dead = (bundledMap, refSet) =>
    [...bundledMap.entries()]
      .filter(([name]) => !refSet.has(name))
      .map(([name, owners]) => ({ name, flows: [...owners] }))
  ...
  return {
    ...
    schemas:  dead(bundled.schemas,  refSchemas),
  }
}
