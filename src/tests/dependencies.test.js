...
function flowB(id, opts = {}) {
  ...
  if (opts.mappingSchemaRefs) f.mappingSchemaRefs = opts.mappingSchemaRefs
  return f
}
...
describe('deadAssets', () => {
  it('flags bundled scripts that no step references', () => {
    ...
    expect(dead.scripts.map(d => d.name)).toEqual(['leftover.groovy', 'orphan.groovy'])
  })

  it('records which flow bundles each dead asset', () => {
    const idx = buildIndex([
      flowB('A', { steps: {}, bundled: { scripts: ['orphanA.groovy'], xslts: [], mappings: [] } }),
      flowB('B', { steps: {}, bundled: { scripts: ['orphanB.groovy'], xslts: [], mappings: [] } }),
    ])
    const dead = deadAssets(idx)
    const byName = Object.fromEntries(dead.scripts.map(d => [d.name, d.flows]))
    expect(byName['orphanA.groovy']).toEqual(['A'])
    expect(byName['orphanB.groovy']).toEqual(['B'])
  })

  it('lists every flow that bundles the same unused asset', () => {
    const idx = buildIndex([
      flowB('A', { steps: {}, bundled: { scripts: ['dup.groovy'], xslts: [], mappings: [] } }),
      flowB('B', { steps: {}, bundled: { scripts: ['dup.groovy'], xslts: [], mappings: [] } }),
    ])
    const dead = deadAssets(idx)
    expect(dead.scripts).toHaveLength(1)
    expect(dead.scripts[0].flows.sort()).toEqual(['A', 'B'])
  })
...
describe('deadAssets with schemas', () => {
  it('flags a bundled XSD that no mmap or adapter references', () => {
    const idx = buildIndex([
      flowB('A', {
        steps: {
          m1: { kind: 'MessageMapping', config: { mappingRef: 'MyMap.mmap' } },
        },
        bundled: { scripts: [], xslts: [], mappings: ['MyMap.mmap'], schemas: ['Used.xsd', 'Orphan.xsd'] },
        mappingSchemaRefs: { 'MyMap.mmap': ['Used.xsd'] },
      }),
    ])
    const dead = deadAssets(idx)
    expect(dead.schemas.map(d => d.name)).toEqual(['Orphan.xsd'])
  })

  it('recognises an EDMX referenced by adapter edmxFilePath as used', () => {
    const idx = buildIndex([
      flowB('A', {
        steps: {
          o: { kind: 'ExternalCall', config: { rawProps: { edmxFilePath: 'edmx/ORG.EDMX' } } },
        },
        bundled: { scripts: [], xslts: [], mappings: [], schemas: ['ORG.EDMX', 'Other.edmx'] },
      }),
    ])
    const dead = deadAssets(idx)
    expect(dead.schemas.map(d => d.name)).toEqual(['Other.edmx'])
  })
})
